...
func main() {
	...
	if os.Getenv("SEC_APP_SALT") == "" {
		...
		os.Setenv("SEC_APP_SALT", autoGeneratedSalt)
	}
...
func handleRegister(scanner *bufio.Scanner, s *auth.Service) {
	if autoGeneratedSalt != "" {
		fmt.Println("\n[Warning] 环境变量 SEC_APP_SALT 未设置。")
		fmt.Printf("已自动生成临时 Salt: %s\n", autoGeneratedSalt)
		fmt.Println("请保存此值并在下次运行前配置环境变量，否则将无法解密本次注册的数据。")
...
func handleLogin(scanner *bufio.Scanner, s *auth.Service, v *vault.Manager) {
	if autoGeneratedSalt != "" {
		...
		fmt.Printf("请配置正确的环境变量: export SEC_APP_SALT=\"...\"\n")
		fmt.Print("按回车键继续...")
		scanner.Scan()
	}
