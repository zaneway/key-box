...
func main() {
	...
	if salt == "" {
		...
		if err != nil {
			fmt.Printf("Error checking user data: %v\n", err)
			os.Exit(1)
		}

		if userCount > 0 {
			missingSaltForExistingDB = true
			fmt.Println("[Warning] 检测到已有账户数据，但未找到 SEC_APP_SALT。")
			fmt.Println("为避免覆盖原密钥，程序没有自动生成新的 Salt。")
			fmt.Println("请将注册或备份时的原 Salt 写入 ~/.key-box.config 后再登录。")
			fmt.Println()
		} else {
			// Generate random salt only for a fresh database.
			b := make([]byte, 16)
...
func handleLogin(scanner *bufio.Scanner, s *auth.Service, v *vault.Manager) {
	if missingSaltForExistingDB {
		fmt.Println("\n[Warning] 当前缺少原 SEC_APP_SALT，已有账户无法解密系统密钥。")
		fmt.Println("请将原 Salt 写入 ~/.key-box.config，或在启动前设置 SEC_APP_SALT。")
		fmt.Print("按回车键继续...")
		scanner.Scan()
	}
